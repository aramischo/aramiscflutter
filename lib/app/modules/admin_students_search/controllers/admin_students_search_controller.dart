...
import 'package:aramisc/app/modules/admin_fees_invoice_list/controllers/admin_fees_invoice_list_controller.dart';
...
import 'package:aramisc/domain/core/model/admin/admin_attendance_model/admin_student_subject_list_response_model.dart';
import 'package:aramisc/domain/core/model/admin/admin_student_model/admin_student_search_response_model.dart';
import 'package:aramisc/domain/core/model/admin/admin_student_model/student_class_list_response_model.dart';
import 'package:aramisc/domain/core/model/admin/admin_student_model/student_section_list_response_model.dart';
...
class AdminStudentsSearchController extends GetxController {
    ...
    try {
      ...
      final response = await BaseClient().getData(
          url: globalRxVariableController.roleId.value == 1
              ? AramiscApi.getAdminClassList
              : AramiscApi.getTeacherClassList,
  ...
  Future<StudentSectionListResponseModel> getStudentSectionList(
    ...
    try {
      ...
      final response = await BaseClient().getData(
          url: globalRxVariableController.roleId.value == 1
              ? AramiscApi.getAdminSectionList(classId: classId)
              : AramiscApi.getTeacherSectionList(classId: classId),
          header: GlobalVariable.header);
  ...
  Future<AdminStudentSubjectListResponseModel> getAdminStudentSubjectList(
    ...
    try {
      ...
      final response = await BaseClient().getData(
        url: globalRxVariableController.roleId.value == 1
            ? AramiscApi.getAdminStudentSubjectList(
                classId: classId, sectionId: sectionId)
            : AramiscApi.getTeacherStudentSubjectList(
                classId: classId,
                sectionId: sectionId,
              ),
        header: GlobalVariable.header,
  ...
  Future<AdminStudentSearchResponseModel> getSearchStudentDataList({
    ...
    try {
      ...
      final response = await BaseClient().getData(
          url: AramiscApi.getAdminStudentSearchList(
            classId: classId,
            sectionId: sectionId,
            rollNo: rollNo,
            name: name,
