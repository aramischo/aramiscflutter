...
import 'package:aramisc/domain/core/model/admin/admin_attendance_model/admin_sub_search_attendance_response_model.dart';
...
class AdminSubjectAttendanceSearchListController extends GetxController {
    ...
    try {
      ...
      final response = await BaseClient().postData(
        url: globalRxVariableController.roleId.value == 1
            ? AramiscApi.getAdminSubjectSearchAttendanceList(
                classId: studentClassId,
                sectionId: studentSectionId,
                subjectId: studentSubjectId,
                attendanceDate: studentAttendanceDate,
              )
            : AramiscApi.getTeacherSubjectSearchAttendanceList(
                classId: studentClassId,
                sectionId: studentSectionId,
                subjectId: studentSubjectId,
                attendanceDate: studentAttendanceDate,
  ...
  Future<PostRequestResponseModel> uploadAttendance() async {
    try {
      ...
      final response = await BaseClient().postData(
          url: globalRxVariableController.roleId.value == 1
              ? AramiscApi.adminSubmitSubjectWiseStudentAttendance
              : AramiscApi.teacherSubmitSubjectWiseStudentAttendance,
          header: GlobalVariable.header,
          payload: {
            'class': classId.value,
  ...
  Future<PostRequestResponseModel> markUnMarkHoliday(
    ...
    try {
      ...
      final response = await BaseClient().postData(
          url: globalRxVariableController.roleId.value == 1
              ? AramiscApi.adminAttendanceMarkUnMarkHolyDay
              : AramiscApi.teacherSubjectWiseAttendanceMarkUnMarkHolyDay,
          header: GlobalVariable.header,
          payload: {
