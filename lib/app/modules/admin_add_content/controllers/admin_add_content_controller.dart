...
class AdminAddContentController extends GetxController {
  ...
  void pickFile() async {
    ...
    var file = await HelperFunctions.pickFile();

    if (file != null) {
      contentFile.value = File(file.path);
    } else {
  ...
  Future<void> uploadContent() async {
    try {
      ...
      debugPrint('${Uri.parse(AramiscApi.postAdminContent)}');
      final request = http.MultipartRequest(
          'POST',
          Uri.parse(globalRxVariableController.roleId.value == 1
              ? AramiscApi.postAdminContent
              : AramiscApi.postTeacherContent));
