...
import 'package:aramisc/app/modules/virtual_class_list/views/virtual_calss_join.dart';
...
import 'package:aramisc/domain/core/model/online_class/zoom/zoom_meeting_list_response_model.dart';
...
class VirtualClassListController extends GetxController {
  ...
  Future<void> openZoom(
    ...
    if (status == "JOIN" || status == 'START') {
      final url = AramiscApi.getJoinMeetingUrlApp(meetingID: meetingId);

      if (await canLaunch(url)) {
        await launch(url);
      } else {
        final webUrl = AramiscApi.getJoinMeetingUrlWeb(meetingID: meetingId);
  ...
  void _detectingUrl(String status) {
    switch (status) {
      case 'jitsi':
        url = AramiscApi.jitsiClassList;
        title = "Jitsi";
        break;
      case 'jitsi_meeting':
        url = AramiscApi.jitsiMeetingList;
        title = "Jitsi";
        break;
      case 'zoom':
        url = AramiscApi.zoomClassList;
        title = "Zoom";
        break;
      case 'big_blue_button':
        url = AramiscApi.bigBlueButtonClassList;
        title = "Big Blue Button";
        break;
      case 'big_blue_button_meeting':
        url = AramiscApi.bigBlueButtonMeetingList;
        title = "Big Blue Button";
        break;
      case 'google_meet_class':
        url = AramiscApi.googleMeetClassList;
        title = "Google Meet";
        break;
      case 'google_meet_meeting':
        url = AramiscApi.googleMeetMeetingList;
        title = "Google Meet";
        break;
      case 'zoom_meeting':
        url = AramiscApi.zoomMeetingList;
        title = "Zoom";
        break;
    }
  }
