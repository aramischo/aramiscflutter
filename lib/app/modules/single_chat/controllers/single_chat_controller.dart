...
import 'package:aramisc/app/modules/blocked_users/controllers/blocked_users_controller.dart';
...
import 'package:aramisc/domain/core/model/chat/conversation_model/single_chat_list_response_model.dart';
import 'package:aramisc/domain/core/model/chat/file_list_response_model/file_list_response_model.dart';
import 'package:aramisc/domain/core/model/chat/search_chat_user/search_chat_user.dart';
import 'package:aramisc/domain/core/model/chat/single_chat_user_list_response_model/single_chat_user_list_response_model.dart';
...
class SingleChatController extends GetxController {
  ...
  Future<SingleChatListResponseModel> singleChatSend() async {
    try {
      ...
      // debugPrint('${Uri.parse(AramiscApi.sendSingleChat)}');
      final request =
          http.MultipartRequest('POST', Uri.parse(AramiscApi.sendSingleChat));
  ...
  Future<SingleChatListResponseModel> getChatConversationList(
    ...
    try {
      ...
      final response = await BaseClient().postData(
          url: AramiscApi.getSingleChatList(userId: userId),
  ...
  Future<FileListResponseModel> getSingleChatFileList(
    ...
    try {
      ...
      final response = await BaseClient().getData(
        url: AramiscApi.getSingleChatFileList(userID: userId),
        header: GlobalVariable.header,
      );
  ...
  Future<void> forwardSingleChat({
    ...
    try {
      ...
      final response = await BaseClient().postData(
          url: AramiscApi.forwardSingleChat(userId: userId, messageId: messageId),
          header: GlobalVariable.header);
  ...
  Future<PostRequestResponseModel> deleteSingleChat(
    ...
    try {
      ...
      final response = await BaseClient().postData(
          url: AramiscApi.deleteSingleChat(messageId: messageId),
          header: GlobalVariable.header);
      PostRequestResponseModel postRequestResponseModel =
          PostRequestResponseModel.fromJson(response);
  ...
  void pickFile() async {
    ...
    if (tempFile != null) {
      singleChatPickImage.value = File(tempFile.path);
    } else {
