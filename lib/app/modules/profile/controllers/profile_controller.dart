...
import 'package:aramisc/app/utilities/file_downloader/file_download_utils.dart';
import 'package:aramisc/app/utilities/widgets/button/primary_button.dart';
import 'package:aramisc/app/utilities/widgets/common_widgets/custom_browse_icon.dart';
import 'package:aramisc/app/utilities/widgets/common_widgets/text_field.dart';
...
import 'package:aramisc/domain/core/model/post_request_response_model.dart';
import 'package:aramisc/domain/core/model/profile/profile_others_model.dart';
import 'package:aramisc/domain/core/model/profile/profile_parents_model.dart';
import 'package:aramisc/domain/core/model/profile/profile_personal_model.dart';
import 'package:aramisc/domain/core/model/profile/profile_transport_model.dart';
import 'package:aramisc/domain/core/model/students_document_response_model/students_document_response_model.dart';
...
import '../../../utilities/app_functions/helper_functions.dart';
...
class ProfileController extends GetxController {
    ...
    try {
      ...
      final response = await BaseClient().getData(
        url: globalRxVariableController.roleId.value == 2
            ? AramiscApi.profilePersonal()
            : AramiscApi.getSingleStudentProfile(studentId: studentId!),
  ...
  void fetchProfileParentsData({int? studentId}) async {
    try {
      ...
      final response = await BaseClient().getData(
          url: globalRxVariableController.roleId.value == 2
              ? AramiscApi.profileParents()
              : AramiscApi.getSingleParentProfile(studentId: studentId!),
          header: {
  ...
  void fetchProfileTransportData({int? studentId}) async {
    try {
      ...
      final response = await BaseClient().getData(
        url: globalRxVariableController.roleId.value == 2
            ? AramiscApi.profileTransport()
            : AramiscApi.getSingleStudentTransportData(studentId: studentId!),
        header: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
  ...
  void fetchProfileOthersData({int? studentId}) async {
    try {
      ...
      final response = await BaseClient().getData(
        url: globalRxVariableController.roleId.value == 2
            ? AramiscApi.profileOthers()
            : AramiscApi.getSingleStudentOthersData(studentId: studentId!),
        header: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
          'Authorization': Get.find<GlobalRxVariableController>().token.value!,
  ...
  Future<StudentDocumentsResponseModel?> getAllDocumentList(
    ...
    try {
      ...
      final response = await BaseClient().getData(
        url: globalRxVariableController.roleId.value == 2
            ? AramiscApi.profileDocumentGet()
            : AramiscApi.getSingleStudentDocumentsData(studentId: studentId!),
        header: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
          'Authorization': Get.find<GlobalRxVariableController>().token.value!,
  ...
  void pickFile() async {
    ...
    var tempFile = await HelperFunctions.pickFile();

    if (tempFile != null) {
      file.value = File(tempFile.path);
    } else {
  ...
  void uploadDocuments() async {
    try {
      ...
      final request = http.MultipartRequest(
          'POST', Uri.parse(AramiscApi.studentUploadDocuments));
  ...
  Future<void> deleteDocument(
    ...
    try {
      ...
      final response = await BaseClient().getData(
          url: AramiscApi.profileDocumentDelete(documentId: documentId),
          header: GlobalVariable.header);
