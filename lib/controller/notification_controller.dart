...
class NotificationController extends GetxController {
  ...
  Future<UserNotificationList> getNotifications() async {
    ...
    try {
      ...
      final response = await http.get(
          Uri.parse(AramiscApi.getMyNotifications(_id)),
  ...
  Future readNotification(int notificationId) async {
    ...
    var response = await http.get(
        Uri.parse(AramiscApi.readMyNotifications(_id.value, notificationId)),
        headers: Utils.setHeader(_token.toString()));
    if (response.statusCode == 200) {
  ...
  Future readAllNotification() async {
    await getIdToken();
    var response = await http.get(
        Uri.parse(AramiscApi.readMyAllNotifications(_id.value)),
        headers: Utils.setHeader(_token.toString()));
    if (response.statusCode == 200) {
      Map notifications = jsonDecode(response.body) as Map;
      bool status = notifications['data']['status'] ?? false;
