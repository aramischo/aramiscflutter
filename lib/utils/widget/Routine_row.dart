...
import 'package:aramisc/utils/model/ScheduleList.dart';
...
class _ClassRoutineState extends State<RoutineRow> {
  ...
  Future<ScheduleList> fetchRoutine(dynamic id,String title) async {
    final response =
    await http.get(Uri.parse(AramiscApi.getRoutineUrl(id)),headers: Utils.setHeader(_token.toString()));
  ...
  Future<ScheduleList> fetchRoutineByClsSec(dynamic id,String title) async {
    final response =
    await http.get(Uri.parse(AramiscApi.getRoutineByClassAndSection(id,classCode,sectionCode)),headers: Utils.setHeader(_token.toString()));
