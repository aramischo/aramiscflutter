...
/// [ListView], a unique [PageStorageKey] must beecified to enable the
/// [CustomExpansionTileCard] to save and restore its expanded state when it is scrolled
/// in and out of view.
///
/// See also:
///
...
class CustomExpansionTileCard extends StatefulWidget {
  ...
  /// A widget to dlay before the title.
  ///
  ...
  /// Additional content dlayed below the title.
  ///
  /// Typically a [Text] widget.
  ...
  /// The widgets that are dlayed when the tile expands.
  ///
  /// Typically [ListTile] widgets.
  final List<Widget> children;

  /// A widget to dlay instead of a rotating arrow icon.
  ...
  ///ecifies if the list tile is initially expanded (true) or collapsed (false, the default).
...
class CustomExpansionTileCardState extends State<CustomExpansionTileCard>
  ...
  void initState() {
    ...
    // headerColor = _controller.drive(_headerColorTween.chain(_colorTween!));
    // _materialColor = _controller.drive(_materialColorTween.chain(_colorTween!));
    // iconColor = _controller.drive(_iconColorTween.chain(_colorTween!));
                    ...
                    child: ListTile(
                      ...
                      title: DefaultTextStyle(
                        style: _isExpanded
                            ...
                            : Get.textTheme.titleMedium!.copyWith(fontSize: 13),
                        child: widget.title,
                      ),
                      ...
                      selectedTileColor: _isExpanded
                          ? Get.theme.primaryColor
                          : Colors.transparent,
                      trailing: RotationTransition(
                        turns: widget.trailing == null || widget.animateTrailing
                            ? _iconTurns ?? const AlwaysStoppedAnimation(0)
