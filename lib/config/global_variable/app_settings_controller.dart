...
import 'package:aramisc/app/database/auth_database.dart';
...
import 'package:aramisc/config/language/controller/language_controller.dart';
import 'package:aramisc/config/language/controller/languages/translated_language.dart';
import 'package:aramisc/domain/core/exceptions/conflict_exception.dart';
import 'package:aramisc/domain/core/exceptions/default.exception.dart';
import 'package:aramisc/domain/core/exceptions/forbidden.exception.dart';
import 'package:aramisc/domain/core/exceptions/unprocessable_exception.dart';
import 'package:aramisc/domain/core/model/settings/general_settings_response_model.dart';
...
class AppSettingsController extends GetxController {
  ...
  void getLang() async {
    debugPrint('LANG URL :: ${AramiscApi.languageList}');

    try {
      languageList.clear();
      languageLoader.value = false;

      final response = await http.get(Uri.parse(AramiscApi.languageList));
  ...
  void getActiveUserLang() async {
    debugPrint('LANG URL :: ${AramiscApi.activeUserLanguageList}');
    ...
    try {
      ...
      final response = await http.get(
        Uri.parse(AramiscApi.activeUserLanguageList),
  ...
  Future<void> getGeneralSettings() async {
    try {
      final response = await http.get(
        Uri.parse(AramiscApi.generalSettings),
        headers: {
  ...
  Future<void> loadData() async {
    try {
      debugPrint('Url ::: ${AramiscApi.service}');
      serviceLoader.value = true;

      // final response = await http.get(Uri.parse(AramiscApi.service),
         // head ers: {'Accept': 'application/json'});
      // var decode = jsonDecode(response.body);
      var decode = true;
      // debug Print('Service Check Response::::::: ${response.body}');
