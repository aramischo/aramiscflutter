...
import 'package:aramisc/utils/model/Vehicle.dart';
...
class _AddVehicleState extends State<AddVehicle> {
  ...
  Future<bool> addVehicleData(String vehicleNo, String model, String driverId,
    ...
    debugPrint(AramiscApi.addVehicle(vehicleNo, model, driverId, note, year));
    response = await dio
        .post(
      AramiscApi.addVehicle(vehicleNo, model, driverId, note, year),
      options: Options(
        headers: {
  ...
  Future<StaffList> getAllStaff() async {
    final response = await http.get(Uri.parse(AramiscApi.driverList),
  ...
  Future<AssignVehicleList> getAllVehicles() async {
    final response = await http.get(Uri.parse(AramiscApi.vehicles),
        headers: Utils.setHeader(_token.toString()));
    if (response.statusCode == 200) {
      var jsonData = jsonDecode(response.body);
