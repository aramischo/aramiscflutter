...
import 'package:aramisc/utils/model/AdminDormitory.dart';
import 'package:aramisc/utils/model/RoomType.dart';
...
class _AddRoomState extends State<AddRoom> {
  ...
  Future<AdminDormitoryList> getAllDormitory() async {
    final response = await http.get(Uri.parse(AramiscApi.dormitoryList),
  ...
  Future<AdminRoomTypeList> getAllRoomType() async {
    final response = await http.get(Uri.parse(AramiscApi.roomTypeList),
        headers: Utils.setHeader(_token.toString()));

    if (response.statusCode == 200) {
      var jsonData = jsonDecode(response.body);
      print("Room types: ${jsonData}");
