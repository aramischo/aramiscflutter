...
import 'package:aramisc/utils/exception/DioException.dart';
import 'package:aramisc/utils/model/Bank.dart';
import 'package:aramisc/screens/fees/model/Fee.dart';
import 'package:aramisc/utils/model/PaymentMethod.dart';
import 'package:aramisc/utils/model/UserDetails.dart';
import 'package:aramisc/utils/widget/ScaleRoute.dart';
import 'package:aramisc/screens/fees/widgets/fees_payment_row_widget.dart';
...
class _FeePaymentMainState extends State<FeePaymentMain> {
  ...
  Future<List<PaymentMethod>> getPaymentMethods() async {
    ...
    final response = await http.get(
        Uri.parse(AramiscApi.paymentMethods(_schoolId)),
  ...
  Future<dynamic> paymentDataSave(String method) async {
    ...
    final response = await http.post(
      Uri.parse(AramiscApi.paymentDataSave),
  ...
  Future paymentCallBack(dynamic method,
    ...
    final response = await http.post(
      Uri.parse(
          AramiscApi.paymentSuccessCallback(status, reference, widget.amount)),
      headers: {
        "Accept": "application/json",
        "Authorization": _token.toString(),
      },
  ...
  Future studentPayment(method) async {
    try {
      ...
      final response = await http.get(
          Uri.parse(AramiscApi.studentFeePayment(
              widget.id.toString(),
...
class _BankOrChequeState extends State<BankOrCheque> {
  ...
  Future<BankList> getBankList() async {
    final response = await http.get(Uri.parse(AramiscApi.bankList),
        headers: Utils.setHeader(_token.toString()));
  ...
  Future submitPayment(context, user) async {
    if (_formKey.currentState!.validate()) {
      if (_file != null) {
        if (widget.paymentType == "Bank Payment") {
          paymentUrl = AramiscApi.childFeeBankPayment(
          ...
          paymentUrl = AramiscApi.childFeeChequePayment(
            amountController.text,
            _userController.selectedRecord.value.classId,
            _userController.selectedRecord.value.sectionId,
            widget.id,
