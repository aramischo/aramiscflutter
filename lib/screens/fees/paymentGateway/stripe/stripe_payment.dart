...
class _StripePaymentScreenState extends State<StripePaymentScreen> {
  ...
  void buy(context) async {
    ...
    String paymentMethodId = paymentIntentRes['id'];
    String status = paymentIntentRes['status'];

    if (status == 'requires_payment_method' || status == 'requires_action' || status == 'requires_confirmation') {
  ...
  Future<Map<String, dynamic>> createPaymentIntent(
    ...
    try {
      ...
      log("payment method check ::: ${stripeCard.toPaymentMethod()}");
  ...
  Future<String?> postCreatePaymentIntent(
    ...
    final url = Uri.parse(stripeServerURL);
    print("URL is $url");
    http.Response response = await http.post(
      ...
      body :{
          'amount': '500',
          'currency': "USD",
           //"payment_method": "pmc_1OdnnOBjijh5dBwQuHdroZi7"
      },
      headers: {
    ...
    final stripeIntentResponse = json.decode(response.body);
    return stripeIntentResponse["client_secret"];
  }
  ...
  Future<Map<String, dynamic>> confirmPayment3DSecure(
    ...
    try {
      ...
      paymentIntentRes_3dSecure = await stripe.confirmPayment(clientSecret,context, paymentMethodId: 'pm_card_visa');

      log("::::::::::::: paymentIntent  :::::: $paymentIntentRes_3dSecure");
