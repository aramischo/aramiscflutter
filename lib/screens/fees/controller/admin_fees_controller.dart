...
import 'package:aramisc/screens/fees/model/FeesAdminAddPaymentModel.dart';
...
class AdminFeesController extends GetxController {
    ...
    await getIdToken().then((value) async {
      try {
        final response = await http.get(
            Uri.parse(AramiscApi.adminFeesAddPayment + '/$invoiceId'),
  ...
  Future processPayment(dio.FormData formData, {BuildContext? context}) async {
    ...
    try {
      ...
      await _dio
          .post(AramiscApi.adminFeesAddPaymentStore,
              data: formData,
              options: dio.Options(
                headers: Utils.setHeader(_token.value.toString()),
              ))
