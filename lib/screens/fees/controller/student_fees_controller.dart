...
import 'package:aramisc/screens/fees/model/FeesRecord.dart';
import 'package:aramisc/screens/fees/model/StudentAddPaymentModel.dart';
...
class StudentFeesController extends GetxController {
  ...
  Future processPayment(dio.FormData formData, {BuildContext? context}) async {
    ...
    try {
      ...
      await _dio
          .post(AramiscApi.studentPaymentStore,
              data: formData,
              options: dio.Options(
  ...
  Future confirmPaymentCallBack(String transactionId) async {
    try {
      ...
      final response = await _dio.get(
          AramiscApi.studentPaymentSuccessCallback + "/Fees/$transactionId",
          options: dio.Options(
            headers: Utils.setHeader(userController.token.value.toString()),
          ));
  ...
  Future<StudentAddPaymentModel> getFeesInvoice(invoiceId) async {
    try {
      ...
      final response = await http.get(
          Uri.parse(AramiscApi.studentFeesAddPayment(invoiceId)),
      ...
      if (response.statusCode == 200) {
        ...
        log("addPaymentModel ::::: ${addPaymentModel.value.paymentMethods?.length}");
  ...
  Future<FeesRecordList> fetchFeesRecord(studentId, recordId) async {
    try {
      isFeesLoading(true);
      final response = await http.get(
          Uri.parse(AramiscApi.feesRecordList + "/$studentId/$recordId"),
          headers: Utils.setHeader(userController.token.value.toString()));
