...
import 'package:aramisc/controller/system_controller.dart';
import 'package:aramisc/screens/chat/views/ChatPageMain.dart';
import 'package:timeago/timeago.dart' as timeago;

// Project imports:
import 'package:aramisc/config/app_config.dart';
import 'package:aramisc/screens/student/Profile.dart';
import 'package:aramisc/utils/CardItem.dart';
import 'package:aramisc/utils/FunctinsData.dart';
...
import 'package:aramisc/utils/server/About.dart';
import 'package:aramisc/utils/server/LogoutService.dart';
...
class _HomeState extends State<Home> {
                                  ...
                                  children: [
                                    ...
                                    ElevatedButton(
                                      onPressed: () async {
                                        var response = await http.get(
                                            Uri.parse(
                                                AramiscApi.readAllNotification(
                                              int.parse(id),
                                            )),
                                            headers: Utils.setHeader(
                                                _token.toString()));
  ...
  showAlertDialog(BuildContext context) {
    ...
    Widget yesButton = TextButton(
      ...
      onPressed: () async {
        ...
        var response = await http.post(Uri.parse(AramiscApi.logout()),
            ...
            child: Container(
              ...
              child: CircleAvatar(
                ...
                child: CachedNetworkImage(
                  imageUrl: AramiscApi.root + (snapshot.data ?? ''),
                  ...
                  errorWidget: (context, url, error) => CachedNetworkImage(
                    imageUrl:
                        AramiscApi.root + 'public/uploads/staff/demo/staff.jpg',
                    imageBuilder: (context, imageProvider) => Container(
                      decoration: BoxDecoration(
                        image: DecorationImage(
                          image: imageProvider,
  ...
  Future<String> getImageUrl(String email, String password, String rule) async {
    ...
    var response = await http.get(Uri.parse(AramiscApi.login(email, password)));

    if (response.statusCode == 200) {
      Map user = jsonDecode(response.body) as Map;
      if (rule == '2') {
        image = AramiscApi.root + user['data']['userDetails']['student_photo'];
      } else if (rule == '3') {
        image = AramiscApi.root + user['data']['userDetails']['fathers_photo'];
      } else {
        image = AramiscApi.root + user['data']['userDetails']['staff_photo'];
      }
    }
    return image == AramiscApi.root
  ...
  Future<int> getNotificationCount(int id) async {
    ...
    var response = await http.get(Uri.parse(AramiscApi.getMyNotifications(id)),
        headers: Utils.setHeader(_token.toString()));
  ...
  Future<UserNotificationList> getNotifications(int id) async {
    final response = await http.get(Uri.parse(AramiscApi.getMyNotifications(id)),
        headers: Utils.setHeader(_token.toString()));
  ...
  void sendTokenToServer(String token) async {
    ...
    final response = await http.get(
        Uri.parse(AramiscApi.setToken(_id ?? '', token)),
...
//       'aramisc', 'aramisc', 'this channel description',
