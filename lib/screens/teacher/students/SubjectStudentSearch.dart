...
import 'package:aramisc/screens/teacher/students/SubjectStudentListScreen.dart';
...
import 'package:aramisc/utils/model/Classes.dart';
import 'package:aramisc/utils/model/Section.dart';
                ...
                sections?.then((sectionValue) {
                  ...
                  subjects?.then((subjectValue) {
                    setState(() {
                      ...
                      url = AramiscApi.getStudentByClassAndSection(
                          classId, sectionId);
                    });
                  });
                });
    ...
    return Scaffold(
      ...
      bottomNavigationBar: GestureDetector(
        ...
        onTap: () {
          ...
          if (name.isNotEmpty) {
            url = AramiscApi.getStudentByName(name, classId, sectionId);
            Navigator.push(
              context,
              ScaleRoute(
            ...
            url = AramiscApi.getStudentByRoll(roll, classId, sectionId);
            Navigator.push(
                context,
                ScaleRoute(
                    page: SubjectStudentListScreen(
            ...
            url = AramiscApi.getStudentBySubject(classId, sectionId, subjectId);
            Navigator.push(
                context,
                ScaleRoute(
                    page: SubjectStudentListScreen(
...
//                          }
        ...
        onChanged: (value) async {
          ...
          sections?.then((sectionValue) {
            ...
            subjects?.then((subjectValue) {
              _selectedSubject = subjectValue.subjects[0].name;
              subjectId = subjectValue.subjects[0].id;
              url = AramiscApi.getStudentByClassAndSection(classId, sectionId);
              setState(() {});
            });
          });
  ...
  Future getAllClass(int id) async {
    final response = await http.get(Uri.parse(AramiscApi.getClassById(id)),
  ...
  Future<SectionList> getAllSection(dynamic id, dynamic classId) async {
    final response = await http.get(
        Uri.parse(AramiscApi.getSectionById(id, classId)),
        headers: Utils.setHeader(_token.toString()));
  ...
  Future<SubjectModelList> getAllSubject(int classId, int sectionId) async {
    final response = await http.post(
        Uri.parse(AramiscApi.getSubjectById(classId, sectionId)),
        headers: Utils.setHeader(_token.toString()));
