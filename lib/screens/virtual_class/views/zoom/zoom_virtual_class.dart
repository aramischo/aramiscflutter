...
class _ZoomVirtualClassState extends State<ZoomVirtualClass> {
  ...
  Future<VirtualClass> getAllMeeting({int? recordId}) async {
    ...
    final _url = widget.type == "class"
        ? Uri.parse(AramiscApi.getVirtualClass(recordId ?? 0, 'zoom'))
        : Uri.parse(AramiscApi.getVirtualMeeting('zoom'));
    print(AramiscApi.getVirtualClass(recordId ?? 0, 'zoom'));
    final response =
                ...
                ElevatedButton(
                  ...
                  onPressed: () async {
                    if (meeting.status == "join" ||
                      ...
                      final _url =
                          AramiscApi.getJoinMeetingUrlApp(mid: meeting.meetingId);
