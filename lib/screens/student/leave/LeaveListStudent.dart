...
import 'package:aramisc/utils/download_dialog.dart';
import 'package:aramisc/utils/model/LeaveAdmin.dart';
import 'package:aramisc/utils/model/StudentLeave.dart';
...
class _LeaveListStudentState extends State<LeaveListStudent> {
  ...
  Future<StudentMyLeavesList> getMyLeaves(var id) async {
    final response = await http.get(Uri.parse(AramiscApi.studentApplyLeave(id)),
  ...
  Future<LeaveAdminList> getApprovedLeaves(var id) async {
    final response = await http.get(Uri.parse(AramiscApi.approvedLeaves(id)),
        headers: Utils.setHeader(_token.toString()));
    if (response.statusCode == 200) {
      var jsonData = jsonDecode(response.body);
  ...
  Future<LeaveAdminList>? getPendingLeaves(var id, purpose) async {
    final response = await http.get(
        Uri.parse(AramiscApi.pendingLeaves(id, purpose)),
        headers: Utils.setHeader(_token.toString()));
    if (response.statusCode == 200) {
      var jsonData = jsonDecode(response.body);
  ...
  Future<LeaveAdminList> getRejectedLeaves(var id) async {
    final response = await http.get(Uri.parse(AramiscApi.rejectedLeaves(id)),
        headers: Utils.setHeader(_token.toString()));
    if (response.statusCode == 200) {
      var jsonData = jsonDecode(response.body);
