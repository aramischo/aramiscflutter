...
import 'package:aramisc/utils/model/UploadedContent.dart';
import 'package:aramisc/utils/widget/StudyMaterial_row.dart';
...
class _StudentStudyMaterialMainState extends State<StudentStudyMaterialMain> {
  ...
  Future<UploadedContentList> fetchAssignment(
    ...
    if (type == 'as') {
      url = AramiscApi.getStudentAssignment(id, recordId);
    } else if (type == 'sy') {
      url = AramiscApi.getStudentSyllabus(id, recordId);
    } else {
      url = AramiscApi.getStudentOtherDownloads(id, recordId);
    }
