...
import 'package:aramisc/screens/chat/models/ChatGroupOpenModel.dart';
...
class ChatGroupOpenController extends GetxController {
  ...
  Future<ChatGroupOpenModel?> getAll() async {
    ...
    try {
      final result = await http.get(
        Uri.parse("${AramiscApi.chatGroupOpen}/$groupId"),
      ...
      if (result.statusCode == 200) {
        ...
        print("Url : ${Uri.parse("${AramiscApi.chatGroupOpen}/$groupId")}");
  ...
  Future chatGroupSetRead(type) async {
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse(AramiscApi.chatGroupSetReadOnly),
  ...
  Future assignRole() async {
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse(AramiscApi.chatGroupAssignRole),
        body: jsonEncode(jsonData),
        headers: Utils.setHeader(_token.toString()),
      );
  ...
  Future deleteChatGroup(groupId) async {
    try {
      await getIdToken().then((value) async {
        final response = await http.post(Uri.parse(AramiscApi.chatGroupDelete),
            headers: Utils.setHeader(_token.toString()),
            body: jsonEncode(
              {
                'group_id': groupId,
  ...
  Future removePeople(userId) async {
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse(AramiscApi.groupRemovePeople),
        body: jsonEncode(jsonData),
        headers: Utils.setHeader(_token.toString()),
      );
  ...
  Future addPeople() async {
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse(AramiscApi.groupAddPeople),
        body: jsonEncode(jsonData),
        headers: Utils.setHeader(_token.toString()),
      );
  ...
  Future leaveGroup() async {
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse(AramiscApi.chatGroupLeave),
        body: jsonEncode(jsonData),
        headers: Utils.setHeader(_token.toString()),
      );
  ...
  Future forwardMessage(Map data, bool isGroup) async {
    ...
    try {
      final response = await http.post(
        Uri.parse(
          isGroup
              ? AramiscApi.chatGroupForwardMessage
              : AramiscApi.chatSingleForwardMessage,
        ),
