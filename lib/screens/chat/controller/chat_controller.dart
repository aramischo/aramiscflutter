...
import 'package:aramisc/screens/chat/controller/pusher_controller.dart';
import 'package:aramisc/utils/Utils.dart';
import 'package:aramisc/utils/apis/Apis.dart';
import 'package:aramisc/screens/chat/models/ChatActiveStatus.dart';
import 'package:aramisc/screens/chat/models/ChatModelMain.dart';
import 'package:http/http.dart' as http;
import 'package:aramisc/screens/chat/models/ChatSettingsModel.dart';
import 'package:aramisc/screens/chat/models/ChatUser.dart';
import 'package:aramisc/screens/chat/models/SearchChatUserModel.dart';
...
class ChatController extends GetxController {
    ...
    try {
      await getIdToken().then((value) async {
        final response = await http.get(
            Uri.parse(AramiscApi.changeChatStatus + "/$type"),
  ...
  Future<ChatUser> getUserStatus() async {
    try {
      ...
      await getIdToken().then((value) async {
        final response = await http.get(
            Uri.parse(AramiscApi.getChatStatus + "/${id.value}"),
            headers: Utils.setHeader(_token.toString()));
        if (response.statusCode == 200) {
  ...
  Future<SearchChatUserModel> searchUser(String keyword) async {
    ...
    try {
      ...
      await getIdToken().then((value) async {
        final response = await http.get(
            Uri.parse(AramiscApi.searchChatUser + "?keywords=$keyword"),
            headers: Utils.setHeader(_token.toString()));
        if (response.statusCode == 200) {
          var jsonData = jsonDecode(response.body);
  ...
  Future<ChatModelMain> getAllChats() async {
    try {
      final response = await http.get(
        Uri.parse(AramiscApi.getChatOpen),
  ...
  Future getChatSettings() async {
    try {
      isLoading(true);
      await getIdToken().then((value) async {
        final response = await http.get(Uri.parse(AramiscApi.chatPermissionGet),
            headers: Utils.setHeader(_token.toString()));
        if (response.statusCode == 200) {
  ...
  Future invitation(to) async {
    ...
    print("Inviting: ${AramiscApi.chatInvitaionOpen}");
    try {
      await getIdToken().then((value) async {
        final response =
            await http.post(Uri.parse(AramiscApi.chatInvitaionOpen), headers: {
          'Accept': 'application/json',
          'Authorization': _token.value.toString(),
        }, body: {
          'to': to.toString()
        });
        print("Invite responce status: ${response.statusCode}");
        if (response.statusCode == 200) {
        } else {
          isLoading(false);
          throw Exception('failed to load');
  ...
  Future getAllBlockedUsers() async {
    try {
      ...
      await getIdToken().then((value) async {
        final response = await http.get(
          Uri.parse(AramiscApi.chatGetBlockedUsers),
          headers: Utils.setHeader(
            _token.toString(),
          ),
        );
  ...
  Future blockUser(action, userId) async {
    ...
    try {
      final response = await http.get(
        Uri.parse(
          AramiscApi.chatUserBlockAction + "/$action/$userId",
        ),
